...
from pytorch_ensembles.metrics import accuracy
...
def main():
    ...
    print('Preparing directory %s' % args.export_dir)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 30 == 0:
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                ...
                path_ = args.export_dir + '/checkpoints'
                os.makedirs(path_, exist_ok=True)
                path_ = (path_ + f'/ImageNet-SGD_{args.arch}_lr{args.lr}_ep{epoch:05}.pt.tar')
                print('\n[CHECKPOINT] Epoch %s acc1 %s acc5 %s Best model %s Save model to %s\n' % (epoch, acc1, acc5, is_best, path_))
    ...
    # remember best acc@1 and save checkpoint
    best_acc1 = max(acc1.cpu(), best_acc1.cpu())
