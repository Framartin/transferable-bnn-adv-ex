...
class ExtendedProjectedGradientDescentPyTorch(ProjectedGradientDescentPyTorch):
    ...
    Also support momentum and translation invariance.
    """
    attack_params = ProjectedGradientDescentPyTorch.attack_params + ['momentum', 'translation_invariant', 'model_target'],
    ...
    def __init__(
        ...
        self.momentum = momentum
        self.translation_invariant = translation_invariant
        # to report target accuracy at each iteration
        self.model_target = model_target
        self._target_total = np.zeros(self.max_iter)
        self._target_correct = np.zeros(self.max_iter)
    ...
    def _generate_batch(
        ...
        # init grad_momentum at begining of a batch
        ...
        for i_max_iter in range(self.max_iter):
            ...
            # compute target accuracy at each iteration
            if self.model_target:
                with torch.no_grad():
                    _, predicted = torch.max(self.model_target(adv_x).data, 1)
                    _, labels = torch.max(targets.data, 1)
                    self._target_total[i_max_iter] += labels.size(0)
                    self._target_correct[i_max_iter] += (predicted == labels).sum().item()
    ...
    def get_target_accuracy_per_iter(self):
        if not self.model_target:
            raise ValueError('model_target must be specified')
        return self._target_correct / self._target_total
